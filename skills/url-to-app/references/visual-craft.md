...
- Compose form controls through shared label/control/help/error wrappers and consistent size variants. Inspect actual control edges with mixed helper/error content; wrapper dimensions alone can hide drift. Check adjacent buttons and input/select variants together using [layout-contracts.md](layout-contracts.md).
...
- At tight widths, reflow the title/action group before forcing short action labels onto awkward multiple lines. Preserve readable text and reachable controls. For contained tables, expose a contextual scrolling cue when needed and verify access to offscreen columns; a clean document-overflow check alone is insufficient.
